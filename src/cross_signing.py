...
class SigningKey:
    ...
    Replaces libolm's `olm.pk.PkSigning`, which vodozemac has no equivalent for.
    PkSigning was only ever an Ed25519 keypair derived from its seed, so this is
    byte-for-byte identical: the same seed yields the same public key and the
    same signatures. That matters because the cross-signing seeds in
    `cross_signing_seeds.json` were generated by the libolm build and the
    homeserver already holds the resulting master key -- deriving anything
    different would reset the bot's identity and force every user to re-verify.
    """

    def __init__(self, seed: bytes) -> None:
        self._key = ECC.construct(curve="Ed25519", seed=seed)
        self._signer = eddsa.new(self._key, "rfc8032")
        self.public_key = _unpadded_b64(self._key.public_key().export_key(format="raw"))
    ...
    def sign(self, message: str) -> str:
        return _unpadded_b64(self._signer.sign(message.encode()))
...
def _load_or_generate_seeds(seeds_file: str) -> tuple[bytes, bytes, bytes]:
    ...
    msk_seed = SigningKey.generate_seed()
    ssk_seed = SigningKey.generate_seed()
    usk_seed = SigningKey.generate_seed()
...
async def bootstrap() -> None:
    ...
    async with aiohttp.ClientSession() as session:
        ...
        master = SigningKey(msk_seed)
        self_signing = SigningKey(ssk_seed)
        _user_signing = SigningKey(usk_seed)
...
def get_msk_public_key() -> str | None:
    ...
    with open(seeds_file) as f:
        seeds = json.load(f)
    msk = SigningKey(base64.b64decode(seeds["master"]))
...
async def cross_sign_user_msk(user_id: str) -> None:
    ...
    seeds = json.loads(await asyncio.to_thread(Path(seeds_file).read_text))
    usk = SigningKey(base64.b64decode(seeds["user_signing"]))
    headers = _matrix_api_headers()
